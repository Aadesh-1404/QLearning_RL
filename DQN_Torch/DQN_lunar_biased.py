...
class DQN_Network(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed):
        ...
        with torch.no_grad():
            ...
            self.fc2.bias.fill_(2.)  # 8
            self.fc3.bias.fill_(3.)  # 2
            self.fc4.bias.fill_(4.)  # 11

    def forward(self, state):
        # w1 = torch.rand(size=(self.state_size, 128))
        # x = torch.matmul(state, w1)
        # x = F.relu(x)
        # w2 = torch.rand(size=(128, 64))
        # x = torch.matmul(state, w2)
        # x = F.relu(x)
        # w3 = torch.rand(size=(64, 64))
        # x = torch.matmul(state, w3)
        # x = F.relu(x)
        # w4 = torch.rand(size=(64, self.action_size))
        # x = torch.matmul(state, w4)

        # self.parameters = [w1, w2, w3, w4]
        x = self.fc1(state)
        x = F.relu(x)
        x = self.fc2(x)
        x = F.relu(x)
...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        if (self.time_step == 0):
            if(len(self.memory) > Batch_size):
                ...
                self.learn(sample_space, gamma, bias)
    ...
    def learn(self, experiences, gamma, bias):
...
plt.ylabel('Score')
plt.xlabel('Episode #')
plt.savefig("Dqn_lunar_torch_bias2.png")
plt.show()
...
def show_video_of_model(agent, env_name):
    ...
    vid = video_recorder.VideoRecorder(
        env, path="video/{}_bias2.mp4".format(env_name))
    agent.dqnetwork_local.load_state_dict(torch.load('checkpoint_bias2.pth'))
