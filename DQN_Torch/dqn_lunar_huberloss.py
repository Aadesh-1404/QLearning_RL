...
class Agent():
    ...
    def huber_loss(self, y_true, y_pred, mask_value, clip_thresh):

        err = y_true - y_pred
        criterion = torch.abs(err) < clip_thresh
        mask_true = (torch.not_equal(y_true, mask_value)).type(torch.float)
        masked_squared_error = torch.square(mask_true*(y_true - y_pred))
        linear_loss = mask_true * \
            (clip_thresh * torch.abs(err) - 0.5 * (clip_thresh ** 2))
        huber_loss = torch.where(criterion, masked_squared_error, linear_loss)

        loss = torch.sum(huber_loss) / torch.sum(mask_true)
        return loss
    ...
    def learn(self, experiences, gamma):
        ...
        loss = self.huber_loss(q_expected, q_targets, 0.0, 1.0)
...
plt.ylabel('Score')
plt.xlabel('Episode #')
plt.savefig("Dqn_lunar_torch_huberloss.png")
plt.show()
...
def show_video_of_model(agent, env_name):
    ...
    vid = video_recorder.VideoRecorder(
        env, path="video/{}_huberloss.mp4".format(env_name))
    agent.dqnetwork_local.load_state_dict(
        torch.load('checkpoint_huberloss.pth'))
