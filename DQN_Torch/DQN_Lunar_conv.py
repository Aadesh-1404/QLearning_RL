...
def dqn(n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if(i_episode % 30 == 0):
            score_avg = np.sum(scores[i_episode-30:i_episode]) / 30
            avg_score.append(score_avg)
            eps_counter.append(i_episode)
        ...
        if (i_episode == n_episodes):
            torch.save(agent.dqnetwork_local.state_dict(),
                       'checkpoint_bias50_eps0_smooth.pth')
            # break
    return scores, avg_score, eps_counter
...
plt.ylabel('Score')
plt.xlabel('Episode #')
plt.savefig("Dqn_lunar_bias50_eps0_smooth.png")
plt.show()
...
def show_video_of_model(agent, env_name):
    ...
    vid = video_recorder.VideoRecorder(
        env, path="video/{}_bias50_eps0_smooth.mp4".format(env_name))
    agent.dqnetwork_local.load_state_dict(
        torch.load('checkpoint_bias50_eps0_smooth.pth'))
