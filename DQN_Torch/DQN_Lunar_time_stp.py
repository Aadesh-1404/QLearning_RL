...
def dqn(n_episodes=2000, max_t=1000, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if(i_episode % 40 == 0):
            score_avg = np.sum(scores[i_episode-40:i_episode]) / 40
            avg_score.append(score_avg)
            eps_counter.append(ts_counter)
        ...
        #                'checkpoint_bias_{}_eps_{}_{}_avg.pth'.format(bias, eps_name, ind))
            # break
    return ts_score, scores, avg_score, ts_counter, eps_counter


# agent = Agent(state_size=8, action_size=4, seed=0)
# scores, avg_scores, eps_counter = dqn()


# fig = plt.figure()
# ax = fig.add_subplot(111)
# plt.plot(eps_counter, avg_scores)
# plt.yticks
# # plt.plot()
# plt.ylabel('Score')
# plt.xlabel('Episode #')
# plt.savefig("Dqn_lunar_test.png")
# plt.show()
...
def show_video_of_model(agent, env_name):
    ...
    vid = video_recorder.VideoRecorder(
        env, path="video/{}_bias_{}_eps_{}_{}_avg.mp4".format(env_name, bias, eps_name, ind))
    agent.dqnetwork_local.load_state_dict(
        torch.load('checkpoint_bias_{}_eps_{}_{}_avg.pth'.format(bias, eps_name, ind)))
...
bias_ = [0, 0, 50, -50]
eps_start_ = [1.0, 0, 0, 0]
eps_end_ = [0.01, 0, 0, 0]
eps_decay_ = [0.995, 0, 0, 0]
...
for i in range(4):
    ...
    for ind in range(3):
        ...
        print("bias, epsilon, jth", bias, eps_name, ind)
        agent = Agent(state_size=8, action_size=4, seed=0)
        ts_scores, scores, avg_scores, ts_counter, eps_counter = dqn(
            eps_start=eps_start_[i], eps_end=eps_end_[i], eps_decay=eps_decay_[i])

        ts_scores = np.array(ts_scores)
        scores = np.array(scores)
        avg_scores = np.array(avg_scores)

        ts_scores_.append(ts_scores)
        scores_.append(scores)
        avg_scores_.append(avg_scores)
        ts_count.append(ts_counter)
        # print(ts_counter)
        # print("HI")
        # print(len(ts_scores_[ind]), len(scores_[ind]), len(avg_scores_[ind]))

    min_ind_ts = np.min(ts_count)
    ts_scores_avg = (ts_scores_[0][:min_ind_ts, ] + ts_scores_[1]
                     [:min_ind_ts, ] + ts_scores_[2][:min_ind_ts, ]) / 3
    scores_avg = (scores_[0] + scores_[1] + scores_[2]) / 3
    avg_scores_avg = (avg_scores_[0] + avg_scores_[1] + avg_scores_[2]) / 3

    print(len(ts_scores_avg), len(scores_avg))
    ts_scores_final.append(ts_scores_avg)
    scores_final.append(scores_avg)
    avg_scores_final.append(avg_scores_avg)
    plt.plot(eps_counter, avg_scores_avg,
             label="bias_{}_epsilon{}".format(bias, eps_name))
...
plt.legend()
plt.savefig("Dqn_lunar_bias_eps_2705_avg.png")
plt.show()
